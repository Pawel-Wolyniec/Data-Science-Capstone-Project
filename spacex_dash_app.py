...
# Create a new column for booster version groups
spacex_df['Booster Version Group'] = spacex_df['Booster Version'].apply(
    lambda x: '1.0' if '1.0' in x else 
              '1.1' if '1.1' in x else 
              'FT' if 'FT' in x else 
              'B4' if 'B4' in x else 
              'B5' if 'B5' in x else 
              'Other'
...
def update_scatter_chart(entered_site, payload_range):
    ...
    # Update y-axis to show success/failure as labels
    fig.update_layout(
        yaxis=dict(
            tickmode='array',
            tickvals=[0, 1],
